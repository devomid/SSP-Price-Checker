...
const ModulesDialog = ({ open, setOpen }) => {
    ...
    const { setModulesPrice } = GeneralState();
    ...
    const handleSwitchChange = (index, event) => {
        ...
        const selectedModuleIndex = selectedModules.indexOf(selectedModule);
        ...
        if (event.target.checked) {
            ...
            setSelectedModules([
                ...selectedModules.slice(0, selectedModuleIndex),
                ...selectedModules.slice(selectedModuleIndex + 1)
            ]);
        };
    };

    const handleAddKits = () => {
        const numbersArray = selectedModules
            ...
            <Dialog sx={{ backgroundColor: 'rgba(255, 255, 255, 0.1)', backdropFilter: 'blur(10px) saturate(280%)' }} open={open} keepMounted onClose={() => setOpen(false)} scroll="paper">
                ...
                <DialogActions>
                    <Stack direction='row' spacing={1} sx={{ width: '100%', m: '10px 5px' }} justifyContent='space-around'>
                        ...
                        <Button sx={{ width: '45%' }} color="primary" variant="outlined" onClick={handleAddKits}>افزودن کیت و ماژول</Button>
                    </Stack>
                </DialogActions>
            </Dialog>
        </Box>
